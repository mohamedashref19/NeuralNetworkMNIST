...
public class NeuralNetwork {
    ...
    public Matrix forwardPass(Matrix input) {
        ...
        finalOutput = ActivationFunctions.applySoftmax(outputLayerInput); // ✅ Softmax for output
    ...
    public void backpropagation(Matrix input, Matrix target) {
        ...
        Matrix inputHiddenWeightsDelta = input.transpose().multiply(hiddenGradient).multiply(learningRate);
        Matrix hiddenOutputWeightsDelta = hiddenLayerOutput.transpose().multiply(outputError).multiply(learningRate);

        weightsHiddenOutput = weightsHiddenOutput.subtract(hiddenOutputWeightsDelta);
        biasOutput = biasOutput.subtract(outputError.multiply(learningRate));

        weightsInputHidden = weightsInputHidden.subtract(inputHiddenWeightsDelta);
        biasHidden = biasHidden.subtract(hiddenGradient.multiply(learningRate));
    }

    // ✅ Training with shuffling + mini-batch
    public void train(Matrix[] inputs, Matrix[] targets, int epochs, int batchSize) {
        ...
        for (int epoch = 0; epoch < epochs; epoch++) {
            ...
            System.out.printf("Epoch %d: Loss = %.6f, Accuracy = %.2f%%%n",
    ...
    public void test(Matrix[] testInputs, Matrix[] testTargets) {
        ...
        System.out.printf("Test results: Loss = %.6f, Accuracy = %.2f%%%n",
